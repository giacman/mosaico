...
watch(filters, (newVal, oldVal) => {
  fetch_get_translations_to_review();
}, {deep: true});


onMounted(async () => {
  await fetch_get_translations_to_review();
...
async function refreshContents() {
  console.log("Refresh contents for");
  try {
    let temp_contents = [];
    let response = await auth_fetch(`/api/get_translations_to_review/?${filters.value}`, {method: 'GET'});
    if (!response.ok) {
...
async function fetch_get_translations_to_review() {
  store.loading = true;
  try {
    const response = await auth_fetch(`/api/get_translations_to_review/?${filters.value}`, {method: 'GET'});
    if (!response.ok) {
      const err = await error_message(response);
      throw new Error(err);
    }
