...
import {auth_fetch, error_message, hideAndCleanup, openContentModal, useStore} from '../utils';
...
async function refreshContents(){
  try{
    let temp_contents = [];
    let response = await auth_fetch(`/api/get_contents_validated/?${filters.value}`, { method: 'GET' });
    if (!response.ok) {
...
async function fetchGetContents(next_url = null) {
	...
	try {
		...
		if(next_url)
			...
			response = await auth_fetch(`/api/get_contents_validated/?${filters.value}`, { method: 'GET' });
		if (!response.ok) {
			const err = await error_message(response);
			throw new Error(err);
		}
...
<template>
  ...
  <div v-if="next" class="text-center my-3">
    <button class="btn btn-secondary" @click="fetchGetContents(next)">Load more</button>
  </div>
