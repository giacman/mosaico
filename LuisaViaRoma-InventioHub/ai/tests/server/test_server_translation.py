...
from models.homepage.classes import Homepage, HomepageOut

load_dotenv()

from utils.task import GenerationTask, TranslationTask, Translation, TaskState, Language
...
if __name__ == "__main__":
    # Configurazione
    a=json.dumps(HomepageOut.model_json_schema())
    ...
    with open("./tests/outputs/generation/newsletter/output_1.json", encoding="utf-8") as f:
        task = TranslationTask.model_validate(json.load(f))

    for i, llm_name in enumerate(MODEL_ENTRIES):
        task.llm_name = llm_name
        task.translations = [Translation(id=i, state=TaskState.SENT, language=Language(id=0, name="Italian",lang_alpha2="it",country_alpha2="IT")), Translation(id=i+1, state=TaskState.SENT,language=Language(id=1, name="Swedish",lang_alpha2="sv",country_alpha2="SE"))]
        process_single_file("translation", task, i)
